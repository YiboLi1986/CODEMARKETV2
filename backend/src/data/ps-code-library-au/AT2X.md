...
// This logical unit calculates swings ('Physical AT tower #2-S04', 'Physical AT tower #2-S14', 'Physical AT tower #2-S24', 'Physical AT tower #2-S34', 'Physical AT tower #2-S44', 'AT#2-MGO/LVD SWING Pre-Offset') for crude unit 'Physical AT tower #2'.
// Original streams (list) from the 'Physical AT tower #2' unit are written to variables. 
// The parameter values ​​entered by users on the 'AT2X' unit are also read to variables. 
// The calculation result is updating the splitter parameters ('Splitter LSRN-SRN2','Splitter SRN-HYNA2', 'Splitter SRN-STOVE2', 'Splitter ST-DIESEL2', 'Splitter DIESEL-MGO2', 'MGO/LVD2 Splitter'). Each splitter divides the original stream into 2 streams and going Up or Down. Splitter accepting values in percentages: 0-100%.
// The final products from 'Physical AT tower #2' are combining in the mixers ('Mixer LSRN2', 'Mixer SRN2', 'Mixer Hy Nap2', 'Mixer STOVE2', 'Mixer DIESEL2', 'MGO2 Swing Mixer', 'LVD2 Swing Mixer').
# Business Description
// The atmospheric unit separates the crude into fractions, from light at the top to heavy at the bottom. Between the fractions there are so-called swing cuts, which can go up and down between certain cuts. The user can control the swings to achieve certain indicators: the total yield of the product and/or the target quality. In this example, the cuts with swings are mixing through mixer units, and the division of the swings is controlled on the splitter units. This is done in order to assign the responsibility for the calculation of the mixing of qualities to the AUS mixers (automatically using the appropriate blend rules and correlation libraries), otherwise such a calculation would have to be done in the JavaScript code.
# Code
```js
// Define Inputs
